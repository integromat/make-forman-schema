...
**Required fields**: an absent required field (not an explicit `null`/`''`) is satisfied by the field's own `default` when one is defined; a missing or `null` default falls through to the `"Field is mandatory."` error. The substituted value is then type-checked and conditions nested branches exactly like a provided one, so a default can surface a mandatory field that the old early return masked.
