...
describe('Forman Schema Manifest Validation', () => {
    ...
    it.only('should validate openai-gpt-3 manifest for Create Completion', async () => {
        const openAiGpt3CreateCompletionMock = JSON.parse(
            readFileSync('./test/mocks/openai-gpt-3-create-completion.json').toString(),
        );
            ...
            await validateFormanWithDomains(
                {
                    default: {
                        ...
                        schema: openAiGpt3CreateCompletionMock.parameters,
                    },
                    expect: {
                        values: {
                            select: 'chat',
                            ...
                            messages: [
                                {
                                    role: 'system',
                                    content:
                                        'You are a sentiment analysis assistant. Analyze the given text and respond with only one word: Positive, Negative, or Neutral.',
                                },
                                {
                                    role: 'user',
                                    content: '{{1.2}}',
                        ...
                        schema: openAiGpt3CreateCompletionMock.expect,
                    },
                },
                {
                    async resolveRemote(path: string, data: Record<string, unknown>) {
                        switch (path) {
                            case 'api://connections/openai-gpt-3':
                                expect(data).toEqual({});
                                return [{ value: 1, label: 'Mocked Connection' }];
                            case 'rpc://openai-gpt-3@1/getModels?type=chat':
                                expect(data).toEqual({
                                    __IMTCONN__: 1,
                                    select: 'chat',
                                });
