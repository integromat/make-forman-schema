...
export type FormanValidationResult = {
    ...
    /** Resolved schema fields per domain. Only present when validation succeeded. */
    schemas?: Record<string, FormanSchemaField[]>;
    /**
     * Resolved schema fields per domain, present whether or not validation succeeded — including
     * fields revealed by a remote-resolved (`rpc://`) sub-form, which cannot be known from the
     * static schema alone. Lets a caller report which fields it rejected.
     */
